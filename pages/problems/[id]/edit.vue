<template>
  <div class="max-w-4xl mx-auto animate-fade-in">
    <div class="mb-8">
      ...
      <h1 class="text-4xl font-bold gradient-text">✏️ 問題を編集</h1>
    </div>

    <form v-if="loaded" @submit.prevent="submitProblem" class="space-y-6">
      <!-- 問題タイプ -->
      <div class="card">
    ...
    <div v-else class="card text-center py-16 animate-pulse">
...
const submitProblem = async () => {
  try {
    ...
    await $fetch(`/api/problems/${route.params.id}`, {
      method: 'PUT',
      body
    })
...
onMounted(async () => {
  try {
    ...
    // 問題を取得
    const problem: any = await $fetch(`/api/problems/${route.params.id}`)
    ...
    form.value = {
      ...
      genreId: problem.genreId,
      difficulty: problem.difficulty,
      choices: problem.choices || [],
      tags: problem.tags.map((t: any) => t.tagName)
    }
